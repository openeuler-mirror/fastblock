	...
	for {
		...
		default:
			...
			if is_read_len {
				...
				err := binary.Read(reader, binary.LittleEndian, &msg_len) // TODO: check little or big
				if err != nil {
					log.Error(ctx, "Error parsing message length:", err)
					panic("Error parsing message length")
				}
			...
			handleRequest(request, ctx, conn, client)

			// buffer := make([]byte, msg_len)
			// n, err = conn.Read(buffer[read_bytes:])
			// if err != nil {
			// 	log.Error(ctx, "Error reading from connection:", err)
			// 	return
			// }
			...
			// log.Debug(ctx, "handleConnection: read bytes:", n)
