...
enum kfastblock_transport_request_finish_stage {
	KFASTBLOCK_REQUEST_FINISH_STAGE_INIT = 0,
...
static int kfastblock_transport_request_finish_ctx_init(
	...
	bool mark_success)
{
	...
	ctx->object_index = object_index;
	ctx->ret = ret;
	ctx->mark_success = mark_success;
	ctx->stage = KFASTBLOCK_REQUEST_FINISH_STAGE_INIT;
	return 0;
}
	...
	struct kfastblock_transport_request_finish_ctx *ctx)
{
	...
	kfastblock_request_mark_object_complete(
		ctx->kf_req, ctx->object_index, ctx->ret);
	if (ctx->ret)
		ctx->first_error = kfastblock_transport_set_first_error(
			ctx->kf_req, ctx->ret);
	else if (!kfastblock_transport_request_failed(ctx->kf_req))
		ctx->first_error = kfastblock_transport_set_first_error(
			ctx->kf_req,
			kfastblock_transport_refill_dispatch_window(ctx->kf_req));

	ctx->stage = KFASTBLOCK_REQUEST_FINISH_STAGE_STATUS_UPDATED;
	...
	bool drop_unscheduled)
{
	...
	ctx->remaining = atomic_dec_return(&ctx->kf_req->pending_objects);
	if (drop_unscheduled)
		ctx->remaining = kfastblock_transport_drop_unscheduled_pending(
			ctx->kf_req, ctx->remaining);

	ctx->stage = KFASTBLOCK_REQUEST_FINISH_STAGE_PENDING_UPDATED;
	return ctx->remaining;
}

static void kfastblock_transport_finish_request_ctx_if_idle(
	struct kfastblock_transport_request_finish_ctx *ctx)
{
	if (!ctx)
		return;
						  ...
						  int ret)
{
	...
	if (kfastblock_transport_request_finish_ctx_init(
		    &ctx, kf_req, object_index, ret, true))
		return;

	kfastblock_transport_run_request_finish_ctx(&ctx);
}
...
static bool kfastblock_transport_transition_submit_runtime(
	struct kfastblock_transport_submit_ctx *ctx)
{
	if (!ctx)
		return false;
...
static bool kfastblock_transport_transition_submit_prefetch(
	struct kfastblock_transport_submit_ctx *ctx)
{
	if (!ctx)
		return false;
...
static bool kfastblock_transport_transition_submit_batch_prepare(
	struct kfastblock_transport_submit_ctx *ctx)
{
	if (!ctx)
		return false;
...
static bool kfastblock_transport_transition_submit_batch_queue(
	struct kfastblock_transport_submit_ctx *ctx)
{
	if (!ctx)
		return false;
	...
	struct kfastblock_transport_submit_ctx *ctx)
{
	...
	switch (ctx->stage) {
	case KFASTBLOCK_SUBMIT_STAGE_INIT:
		return kfastblock_transport_transition_submit_runtime(ctx);
	case KFASTBLOCK_SUBMIT_STAGE_RUNTIME_READY:
		return kfastblock_transport_transition_submit_prefetch(ctx);
	...
	struct kfastblock_transport_submit_ctx *ctx)
{
	...
	while (!ctx->finished) {
		if (!kfastblock_transport_advance_submit_ctx(ctx))
			break;
	}
	if (ctx->ret < 0)
		return kfastblock_transport_finish_submit_ctx(ctx);
	if (ctx->stage == KFASTBLOCK_SUBMIT_STAGE_BATCH_QUEUED)
		ctx->finished = true;
