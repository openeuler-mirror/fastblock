...
function(compile_library)
  ...
  if(${CC_LIB_SHARED})
    set(CC_LIB_STATIC_SHARED SHARED)
  else()
    set(CC_LIB_STATIC_SHARED STATIC)
  endif()
  ...
  if(NOT CC_LIB_IS_INTERFACE)
    # 创建静态库
    add_library(${_NAME} ${CC_LIB_STATIC_SHARED} "")
    ...
    target_link_libraries(${_NAME}
      ...
      PRIVATE
        ${CC_LIB_LINKOPTS})
    message(STATUS "[1] Compiling ${_NAME} with ${DEFAULT_LINKOPTS} ${CC_LIB_DEPS} ${CC_LIB_LINKOPTS}")
    ...
    # 生成只有头文件的INTERFACE
    add_library(${_NAME} ${CC_LIB_STATIC_SHARED} INTERFACE)
    target_include_directories(${_NAME}
      INTERFACE
      "$<BUILD_INTERFACE:${COMMON_INCLUDE_DIRS}>"
    )
    ...
    target_link_libraries(${_NAME}
      ...
      ${CC_LIB_LINKOPTS}
      ${DEFAULT_LINKOPTS})
      message(STATUS "[2] Compiling ${_NAME} with ${DEFAULT_LINKOPTS} ${CC_LIB_DEPS} ${CC_LIB_LINKOPTS}")
  ...
  if (${CC_LIB_ENABLE_INSTALL})
    message(STATUS "base ld falgs ${BASE_LD_FLAGS}")
    if (${CC_LIB_SHARED})
      set_target_properties(${_NAME}
        PROPERTIES
          INSTALL_RPATH "/usr/local/fastblock/debug/lib:/usr/local/fastblock/debug/lib64:/usr/local/fastblock/debug/lib"
          BUILD_WITH_INSTALL_RPATH TRUE)
    endif()
    install(TARGETS ${_NAME} ARCHIVE DESTINATION lib64)
  endif()
endfunction()

# /usr/local/fastblock/debug/lib:/usr/local/fastblock/debug/lib64:/usr/local/fastblock/debug/lib