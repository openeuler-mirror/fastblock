...
int main(int argc, char *argv[])
{
    ...
    std::cout << "run iscsi_tgt with cpumask " << core_mask.value() << std::endl;
	...
	std::string pid_path = "/var/tmp/fastblock_iscsi_tgt_" + std::to_string(getpid()) + ".pid";
	save_pid(pid_path.c_str());
	auto iscsi_tgt_path = "/var/tmp/fastblock_iscsi_tgt_" + std::to_string(getpid()) + ".sock";
	SPDK_NOTICELOG(
		"pid path is '%s', fastblock_iscsi_tgt socket path is '%s'\n",
		pid_path.c_str(), iscsi_tgt_path.c_str());

	opts.rpc_addr = iscsi_tgt_path.c_str();
    g_app_name = "iscsi_tgt";
