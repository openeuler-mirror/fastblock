...
static void storage_init_complete(void *arg, int rberrno){
	...
	auto& gs = global_storage(shard_id);
	auto *kv = gs.kvs();
...
static void test_disk_log_complete(void *arg, int rberrno){
    ...
    auto shard_id = core_sharded::get_core_sharded().this_shard_id();
	make_disk_log(global_blobstore(shard_id), global_io_channel(core_sharded::get_core_sharded().this_shard_id()), "test", make_log_done, nullptr, shard_id);    
}
...
hello_start(void *arg)
{
    ...
    if(g_mkfs){
        //初始化log磁盘
        blobstore_init(server->bdev_disk, server->osd_uuid, true, disk_init_complete, arg);
    }else{
        blobstore_load(server->bdev_disk, disk_load_complete, arg);
    }
}
