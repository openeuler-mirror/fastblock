...
public:
    ...
    static void load_read_done(void *arg, rblob_rw_result res, int rberrno){
        ...
        if(next_unit > ctx->length)
            ctx->length *= 2;
        SPDK_INFOLOG(blob_log, "load rblob: check res [%d, %lu, %lu]\n", all_valid, valid_size, next_unit);
        if(all_valid && valid_size > 0){
            ctx->pos += valid_size;
            free_buffer_list(ctx->bl);
            SPDK_INFOLOG(blob_log, "load rblob: read pos %lu length %lu\n", ctx->pos, ctx->length);
    ...
    void load(rblob_op_complete cb_fn, void* arg, check_and_handle_data_func check, uint64_t read_unit = default_load_read_size){
        auto load_md_done = [this, read_unit, cb_fn = std::move(cb_fn), check = std::move(check)](void *arg, int lerrno){
            ...
            SPDK_INFOLOG(blob_log, "load rblob: after load md back_pos %lu front_pos %lu\n", back_pos(), front_pos());
...
public:
    void close(rblob_op_complete cb_fn, void* arg) {
      ...
      SPDK_DEBUGLOG(blob_log, "close rolling blob in core %u\n", core_sharded::get_core_sharded().this_shard_id());
...
private:
    ...
    static void delete_blob_complete(void *arg, int rberrno) {
        ...
        if(rberrno){
            SPDK_ERRLOG("delete blob 0x%lx failed:%s\n", ctx->blobid, spdk_strerror(rberrno));
        }
        ctx->cb_fn(ctx->arg, rberrno);
        delete ctx;
    }
