...
void client::emplace_create_clone_from_snapshot_request(
  const std::string snapshot_id,
  const std::string clone_image_name,
  on_response_callback_type&& cb) {
    auto req = std::make_unique<msg::Request>();
    auto* real_req = req->mutable_create_clone_from_snapshot_request();
    real_req->set_snapshot_id(snapshot_id);
    real_req->set_clone_image_name(clone_image_name);
...
void client::emplace_protect_snapshot_request(const std::string snapshot_id, on_response_callback_type&& cb) {
    auto req = std::make_unique<msg::Request>();
    req->mutable_protect_snapshot_request()->set_snapshot_id(snapshot_id);
    auto* ctx = new client::request_context{
      this, std::move(req), std::monostate{}, std::move(cb)};
    enqueue_request(ctx);
}

void client::emplace_unprotect_snapshot_request(const std::string snapshot_id, on_response_callback_type&& cb) {
    auto req = std::make_unique<msg::Request>();
    req->mutable_unprotect_snapshot_request()->set_snapshot_id(snapshot_id);
    auto* ctx = new client::request_context{
      this, std::move(req), std::monostate{}, std::move(cb)};
    enqueue_request(ctx);
}

void client::emplace_delete_image_snapshot_request(const std::string snapshot_id, on_response_callback_type&& cb) {
    auto req = std::make_unique<msg::Request>();
    req->mutable_delete_image_snapshot_request()->set_snapshot_id(snapshot_id);
    auto* ctx = new client::request_context{
      this, std::move(req), std::monostate{}, std::move(cb)};
    enqueue_request(ctx);
}
...
void client::emplace_get_image_metadata_by_name_request(const std::string& pool_name, const std::string& image_name, on_response_callback_type&& cb) {
    auto req = std::make_unique<msg::Request>();
    auto* real_req = req->mutable_get_image_metadata_by_name_request();
    real_req->set_pool_name(pool_name);
    real_req->set_image_name(image_name);
    auto* ctx = new client::request_context{
      this, std::move(req), std::monostate{}, std::move(cb)};
...
void client::emplace_get_snapshot_metadata_by_id_request(const std::string& snapshot_id, on_response_callback_type&& cb) {
    auto req = std::make_unique<msg::Request>();
    req->mutable_get_snapshot_metadata_by_id_request()->set_snapshot_id(snapshot_id);
    auto* ctx = new client::request_context{
      this, std::move(req), std::monostate{}, std::move(cb)};
    enqueue_request(ctx);
}
...
void client::process_response(std::shared_ptr<msg::Response> response) {
    ...
    case msg::Response::UnionCase::kCreateCloneFromSnapshotResponse: {
        SPDK_DEBUGLOG(mon, "Received create clone from snapshot response\n");
        auto& resp = response->create_clone_from_snapshot_response();
        auto& req_ctx = _on_flight_requests.front();
    ...
    case msg::Response::UnionCase::kProtectSnapshotResponse: {
        SPDK_DEBUGLOG(mon, "Received protect snapshot response\n");
        auto& resp = response->protect_snapshot_response();
        auto& req_ctx = _on_flight_requests.front();
        req_ctx->response_data = std::make_unique<client::snapshot_metadata>(snapshot_metadata_from_proto(resp.metadata()));
        req_ctx->cb(to_response_status(resp.errorcode()), req_ctx.get());
        _on_flight_requests.pop_front();
    ...
    case msg::Response::UnionCase::kUnprotectSnapshotResponse: {
        SPDK_DEBUGLOG(mon, "Received unprotect snapshot response\n");
        auto& resp = response->unprotect_snapshot_response();
        auto& req_ctx = _on_flight_requests.front();
        req_ctx->response_data = std::make_unique<client::snapshot_metadata>(snapshot_metadata_from_proto(resp.metadata()));
        req_ctx->cb(to_response_status(resp.errorcode()), req_ctx.get());
        _on_flight_requests.pop_front();
    ...
    case msg::Response::UnionCase::kDeleteImageSnapshotResponse: {
        SPDK_DEBUGLOG(mon, "Received delete image snapshot response\n");
    ...
    case msg::Response::UnionCase::kGetImageMetadataByNameResponse: {
        SPDK_DEBUGLOG(mon, "Received get image metadata by name response\n");
        auto& resp = response->get_image_metadata_by_name_response();
        auto& req_ctx = _on_flight_requests.front();
        req_ctx->response_data = std::make_unique<client::image_metadata>(image_metadata_from_proto(resp.metadata()));
        req_ctx->cb(to_response_status(resp.errorcode()), req_ctx.get());
        _on_flight_requests.pop_front();
    ...
    case msg::Response::UnionCase::kGetSnapshotMetadataByIdResponse: {
        SPDK_DEBUGLOG(mon, "Received get snapshot metadata by id response\n");
