...
int node_configuration_manager::cfg_change_process(int result, raft_index_t rsp_current_idx, std::shared_ptr<raft_node> node){
    ...
    auto create_cfg_entry = [this](utils::context* complete){
        ...
        for(uint64_t i = 0; i < nodes.size(); i++){
            auto info = cfg.add_new_nodes();
            *info = nodes[i];
            SPDK_INFOLOG(pg_group, "in pg %s , new node %d\n", _pg_name.c_str(), info->node_id());
        }
    ...
    case  cfg_state::CFG_CATCHING_UP:
    {
        if(!get_catch_up_node(node->raft_node_get_id()))
            return 0;
        SPDK_INFOLOG(pg_group, "in pg %s , state: %d node_id: %d result: %d\n", 
        ...
        if(all_new_nodes_catch_up()){
            SPDK_INFOLOG(pg_group, "in pg %s , all_new_nodes_catch_up\n", _pg_name.c_str());
            int ret = 0;
    ...
    case cfg_state::CFG_JOINT:
    {
        ...
        SPDK_INFOLOG(pg_group, "in pg %s , state: %d node_id: %d rsp_current_idx:%ld result: %d\n", 
        ...
        if((_new_node_match_size  > get_new_node_size() / 2)
            ...
            SPDK_INFOLOG(pg_group, "in pg %s , all nodes match\n", _pg_name.c_str());
...
bool node_configuration_manager::save_node_configuration(){
    ...
    if(!res){
        SPDK_INFOLOG(pg_group, "in pg %s , serialize node_configuration failed.\n", _pg_name.c_str());
        return false;
    }

    if(_raft->save_node_configuration(value) != 0){
        SPDK_INFOLOG(pg_group, "in pg %s , save_node_configuration failed.\n", _pg_name.c_str());
        return false;
    }
    return true;        
}
...
bool node_configuration_manager::load_node_configuration(){
    ...
    if(!val.has_value()){
        SPDK_INFOLOG(pg_group, "in pg %s , no find node configuration\n", _pg_name.c_str());
        return false;    
    }
