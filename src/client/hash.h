...
namespace {
inline unsigned jenkins_hash(const std::string &str, unsigned length) {
...
inline auto calc_pg_masks(int32_t pg_num) {
    return (1 << cbits(pg_num - 1)) - 1;
}

inline unsigned calc_target(const std::string &sstr, int32_t target_pool_id, int32_t pg_num) {
