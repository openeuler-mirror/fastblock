...
int watch_poller(void *arg)
{
    ...
    if (is_all_done)
    {
        ...
        fmt = boost::format("%1%, mean: %2%us, min: %3%us, max: %4%us, biased stdv: %5%, iops: %6%") % fmt % tick_to_us(mean) % tick_to_us(min) % tick_to_us(max) % tick_to_us(biased_stdv) % (ctx->total_io_count / (iops_dur / ::spdk_get_ticks_hz()));
