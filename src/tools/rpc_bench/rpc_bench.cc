...
int g_id{-1};
namespace {
static bool all_rpc_done{false};
...
public:
    ...
    void can_i_stop(
      google::protobuf::RpcController* controller,
      const ::rpc_bench::stop_request* request,
      ::rpc_bench::stop_response* response,
      ::google::protobuf::Closure* done) override {
        response->set_ok(all_rpc_done);
        done->Run();
    }
};
...
struct stop_stack {
    std::unique_ptr<rpc_bench::stop_request> req{std::make_unique<rpc_bench::stop_request>()};
    std::unique_ptr<rpc_bench::stop_response> resp{std::make_unique<rpc_bench::stop_response>()};
    std::unique_ptr<msg::rdma::rpc_controller> ctrlr{std::make_unique<msg::rdma::rpc_controller>()};
    google::protobuf::Closure* cb{nullptr};
};
...
struct connection_context {
    ...
    int64_t call_id{0};
    size_t index{0};
    std::unique_ptr<stop_stack> stop_rpc{nullptr};
    bool can_i_stop{false};
...
std::unique_ptr<utils::simple_poller> request_stop_poller{nullptr};
std::chrono::system_clock::time_point iops_start_at{};
std::chrono::system_clock::duration iops_duration{};
}
...
void read_conf() {
    ...
    for (; eps_it != conf_eps.end(); ++eps_it) {
        ...
        for (; list_it != eps_list.end(); ++list_it) {
            auto& ep = ctx.endpoints.emplace_back(
              current_index,
              list_it->second.get_child("host").get_value<std::string>(),
              list_it->second.get_child("port").get_value<uint16_t>());

            if (list_it->second.count("factor") != 0) {
                ep.factor = list_it->second.get_child("factor").get_value<size_t>();
            }
        }
    }
...
void print_result() {
    ...
    std::vector<double> durations{};
    for (auto& ctx : conn_ctxs) {
        durations.insert(durations.end(), ctx->durations.begin(), ctx->durations.end());
    }
    ...
    for (auto lat_tag : latency_tag) {
        ...
        SPDK_DEBUGLOG(rpc_bench, "p%f at %ld\n", lat_tag, lat_at);
    ...
    auto iops_dur_sec = static_cast<double>(iops_duration.count()) / 1000 / 1000 / 1000;
    auto total_io = ctx.io_count * conn_ctxs.size();
    auto iops_val = total_io / iops_dur_sec;

    fmt = boost::format("%1%, mean: %2%us, min: %3%us, max: %4%us, biased stdv: %5%, iops: %6%, iops_dur_sec: %7%, total_io_count: %8%, io_depth: %9%")
      % fmt % (mean / 1000) % (min / 1000)
      % (max / 1000) % (biased_stdv / 1000)
      % iops_val % iops_dur_sec % total_io % (ctx.io_depth);
...
void on_client_stopped(void* arg) {
    ...
    if (done_counter == rpc_clients.size()) {
        ...
        for (auto& srv : rpc_servers) {
            SPDK_DEBUGLOG(rpc_bench, "stop the rpc server\n");
            srv->stop([] () {
...
void on_stop_response(connection_context* conn_ctx) {
    if (is_terminated) { return; }

    auto* stack_ptr = conn_ctx->stop_rpc.get();
    if (stack_ptr->ctrlr->Failed()) {
        SPDK_ERRLOG("stop request rpc failed, %s\n", stack_ptr->ctrlr->ErrorText().c_str());
        throw std::runtime_error("stop request rpc failed");
    }

    if (stack_ptr->resp->ok()) {
        conn_ctx->can_i_stop = true;
        SPDK_NOTICELOG("connection with index %lu can stop\n", conn_ctx->index);
    }

    conn_ctx->stop_rpc = nullptr;
}

static int request_stop_poll(void* arg) {
    if (is_terminated) {
        return SPDK_POLLER_IDLE;
    }

    auto all_can_stop = std::all_of(
      conn_ctxs.begin(),
      conn_ctxs.end(),
      [] (std::unique_ptr<connection_context>& ctx) {
          return ctx->can_i_stop;
      }
    );

    if (all_can_stop) {
        SPDK_NOTICELOG("start stopping rpc_bench\n");
        is_terminated = true;
    ...
    for (auto& conn_ctx : conn_ctxs) {
        if (conn_ctx->stop_rpc) {
            continue;
        }

        conn_ctx->stop_rpc = std::make_unique<stop_stack>();
        conn_ctx->stop_rpc->cb = google::protobuf::NewCallback(on_stop_response, conn_ctx.get());
        conn_ctx->stub->can_i_stop(
          conn_ctx->stop_rpc->ctrlr.get(),
          conn_ctx->stop_rpc->req.get(),
          conn_ctx->stop_rpc->resp.get(),
          conn_ctx->stop_rpc->cb);
    }
...
void on_client_io_done(void* arg) {
    ++done_counter;
    SPDK_DEBUGLOG(rpc_bench, "done_counter is %ld, conn_ctxs.size() is %ld\n", done_counter, conn_ctxs.size());
    if (done_counter == conn_ctxs.size()) {
        iops_duration = std::chrono::system_clock::now() - iops_start_at;
        SPDK_NOTICELOG("all rpc request finished\n");
        done_counter = 0;
        all_rpc_done = true;
        print_result();
        request_stop_poller->register_poller(request_stop_poll, nullptr, 10000); // 10s
    }
}
...
void on_pong(call_stack* stack_ptr) {
    ...
    auto resp_id = static_cast<size_t>(stack_ptr->resp->id());
    conn_ctx->durations[resp_id] = static_cast<double>((std::chrono::system_clock::now() - stack_ptr->start_at).count());
    SPDK_INFOLOG(
      rpc_bench,
      "[%ld] received pong id %ld, total %ld\n",
      index, resp_id, ctx.io_count - 1);
    auto* call_stacks = &(conn_ctx->call_stacks);
    conn_ctx->call_stacks.pop_front();
    if (resp_id >= ctx.io_count - 1) {
        SPDK_NOTICELOG("[%ld] all rpc finished\n", index);
        ::spdk_thread_send_msg(rpc_bench_thread, on_client_io_done, nullptr);
        return;
    }

    if (conn_ctx->call_id >= static_cast<int64_t>(ctx.io_count)) {
        return;
    }
...
void start_ping_client() {
    ...
    for (; ep_it != ctx.endpoints.end(); ++ep_it) {
        ...
        for (size_t i{0}; i < ep_it->factor; ++i) {
            current_cli->emplace_connection(
              ...
              [core_no, ep_it] (bool is_ok, std::shared_ptr<msg::rdma::client::connection> conn) {
                  ...
                  conn_ctx->durations.resize(ctx.io_count);
        ...
        iops_start_at = std::chrono::system_clock::now();
    }
}
...
void on_rpc_bench_start(void* arg) {
    ...
    request_stop_poller = std::make_unique<utils::simple_poller>(rpc_bench_thread);
